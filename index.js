...
app.post("/signup", (req, res) => {
  ...
  db.query(
    "INSERT INTO credit_card (card_no, holder_name, exp_date, cvv) VALUES (?,?,?,?)",
    [creditCardNo, holdreName, expDate, cvv],
    (err, result) => {
      if (err) {
        console.log(err);
          ...
          (err, result) => {
            if (err) {
              ...
              var mailOptions = {
                ...
                to: email,
                subject: "Welcome to HokieWheels🚘💚",
                // text: "Welcome to HokieWheels! 😃💚\
                // From all of us at HokieWheels, we wish you a splendid experience.\
                // \
                // Ride away!\
                // \
                // Regards,\
                // The HokieWheels team"
                text: "Welcome to HokieWheels!\nFrom all of us at HokieWheels, we wish you a splendid experience Ride away!\n\n\n\nRegards,\nThe HokieWheels team",
              };
...
app.post("/add-car", authorizeOffice, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      db.query(
        "INSERT INTO car_status (plate_id) VALUES (?)",
        [plateId],
        (err, result) => {
          if (err) {
            console.log(err);
          ...
          if (photo1 !== "")
            db.query(
              "INSERT INTO car_photos (plate_id, photo) VALUES (?,?)",
              [plateId, photo1],
              (err, result) => {
                if (err) {
                  console.log(err);
...
app.post("/add-reservation", authorizeCustomer, (req, res) => {
  ...
  if (payNow === "true")
    query =
      "INSERT into rental (license, plate_id, pickup_date, return_date, payment_date) VALUES (?,?,?,?, CURDATE())";
  else
    query =
      "INSERT into rental (license, plate_id, pickup_date, return_date) VALUES (?,?,?,?)";
  db.query(query, [license, plateId, pickupDate, returnDate], (err, result) => {
    if (err) return res.send({ message: err });

    db.query(
      "INSERT INTO car_status (plate_id, status_code, status_date) VALUES (?,?,?)",
      [plateId, 3, pickupForCarStatus],
      (err, result) => {
        if (err) return res.send({ message: err });
        db.query(
          "INSERT INTO car_status (plate_id, status_code, status_date) VALUES (?,?,?)",
          [plateId, 0, returnForCarStatus],
          (err, result) => {
            if (err) return res.send({ message: err });
            res.send({ success: true });
          }
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result[0].office_id == office_id) {
        ...
        res.send({
          success: false,
          message: "You are not authorized to change the status of this car",
        });
      }
    }
  );
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (result[0].office_id == office_id) {
        ...
        db.query(
          "INSERT INTO `car_status`(`plate_id`, `status_code`, `status_date`) VALUES (?,?,CURRENT_TIMESTAMP())",
          [plate_id, status],
          (err, result) => {
...
app.post("/get-office-reservation", authorizeOffice, (req, res) => {
  ...
  ///get the reservation info from the database
  db.query(
    "SELECT * , ((DATEDIFF(return_date,pickup_date)+1)*price ) as revenue from rental JOIN vehicle ON rental.plate_id = vehicle.plate_id join branch on vehicle.office_id = branch.office_id JOIN customer ON rental.license = customer.license where branch.office_id = ?",
    [id],
    (err, result) => {
      if (err) return res.send({ message: err });
...
app.post("/show-avaialable-cars", authorizeCustomer, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) return res.send({ message: err });
      if (result != null) result = result.filter((car) => car.status_code == 0);
      res.send({ cars: result, message: "success" });
    }
  );
});
